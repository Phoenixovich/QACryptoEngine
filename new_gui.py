...
class QKDGui(QWidget):
    ...
    def setup_ui(self):
        ...
        # Add encrypted message text box below MITM label
        self.mitm_encrypted_text = QTextEdit(self)
        self.mitm_encrypted_text.setReadOnly(True)
        self.mitm_encrypted_text.setFixedHeight(120)
        self.mitm_encrypted_text.setFont(QFont('Arial', 10))
        self.mitm_layout.addWidget(self.mitm_encrypted_text, alignment=Qt.AlignCenter)
    ...
    def update_message_anim(self):
        ...
        if anim['step'] >= 30:
            ...
            # MITM interception: show encrypted message below MITM button if MITM is active
            if anim['mitm'] and self.mitm_show_output:
                current = self.mitm_encrypted_text.toPlainText()
                new_entry = f"MITM intercepted (encrypted):\n{encrypted.decode()}\n"
                self.mitm_encrypted_text.setPlainText(current + ("\n" if current else "") + new_entry)
                self.mitm_encrypted_text.verticalScrollBar().setValue(self.mitm_encrypted_text.verticalScrollBar().maximum())
            else:
                self.mitm_encrypted_text.clear()
            # Decrypt and display
